...
grep -q pip_install_module "$INSTALLER"
grep -q break-system-packages "$INSTALLER"
echo "  OK"

echo "[7/8] grabbit install subcommand present..."
grep -q 'do_global_install' "$GRABBIT"
grep -q '_maybe_complete_global_install' "$GRABBIT"
grep -q 'INCLUDE_SYSTEM_PACKAGES' "$GRABBIT"
grep -q 'INCLUDE_DE_PACKAGES' "$GRABBIT"
grep -q '_package_is_base' "$GRABBIT"
grep -q '_package_is_desktop_environment' "$GRABBIT"
grep -q 'filter_de_packages' "$GRABBIT"
grep -q '_package_is_downstream_distro' "$GRABBIT"
grep -q '_get_downstream_pacman_repos' "$GRABBIT"
grep -q '_package_is_downstream_distro' "$SCRIPT_DIR/grabbit_gui.py"
grep -q '_filter_base_packages' "$SCRIPT_DIR/grabbit_gui.py"
grep -q '_filter_de_packages' "$SCRIPT_DIR/grabbit_gui.py"
grep -q '_package_is_desktop_environment' "$SCRIPT_DIR/grabbit_gui.py"
grep -q 'include_system_var' "$SCRIPT_DIR/grabbit_gui.py"
grep -q 'include_de_var' "$SCRIPT_DIR/grabbit_gui.py"
